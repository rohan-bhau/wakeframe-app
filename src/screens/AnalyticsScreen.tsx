...
type TimelineActivity = Activity & { date: string; state: Filter; actualMinutes: number; breaks: AnalyticsBreak[] };
...
export function AnalyticsScreen({ uid }: { uid: string }) {
  ...
  const entries = dates.flatMap((date) => activities
    ...
    .map((activity) => {
      ...
      const instance = instances.find((item) => item.activityId === activity.id && item.date === dateKey);
      const breaks = breakLogs.filter((item) => item.date === dateKey && (item.activityId === activity.id || item.activityInstanceId === `${activity.id}_${dateKey}`));
      return { ...activity, date: dateKey, state: getDisplayState(instance, activity, date, now), breaks, actualMinutes: getActualMinutes(instance, breaks, date, now) };
    }))
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.periods} accessibilityRole="tablist">
        {(['day', 'week', 'month'] as Period[]).map((item) => <Pressable accessibilityRole="tab" accessibilityState={{ selected: period === item }} key={item} onPress={() => changePeriod(item)} style={[styles.period, period === item && styles.selectedPeriod]}><Text style={[styles.periodText, period === item && styles.selectedPeriodText]}>{item[0].toUpperCase() + item.slice(1)}</Text></Pressable>)}
      </View>
      ...
      {!loading && !error ? <>
        ...
        <View style={styles.panel}>
          ...
          <View style={styles.filters}>{filters.map((item) => <Pressable accessibilityRole="button" accessibilityState={{ selected: filter === item }} key={item} onPress={() => changeFilter(item)} style={[styles.filterButton, filter === item && styles.selectedFilter]}><Text style={[styles.filterText, filter === item && styles.selectedFilterText]}>{filterLabels[item]}</Text></Pressable>)}</View>
        </View>
...
function Pagination({ page, pageCount, onChange }: { page: number; pageCount: number; onChange: (page: number) => void }) { return <View style={styles.pagination}><Pressable accessibilityLabel="Previous page" accessibilityRole="button" disabled={page === 0} onPress={() => onChange(page - 1)} style={[styles.pageButton, page === 0 && styles.disabledButton]}><Text style={styles.pageButtonText}>Previous</Text></Pressable><Text style={styles.pageText}>{page + 1} / {pageCount}</Text><Pressable accessibilityLabel="Next page" accessibilityRole="button" disabled={page === pageCount - 1} onPress={() => onChange(page + 1)} style={[styles.pageButton, page === pageCount - 1 && styles.disabledButton]}><Text style={styles.pageButtonText}>Next</Text></Pressable></View>; }
...
function getPeriodDates(now: Date, period: Period) { const start = new Date(now); start.setHours(0, 0, 0, 0); if (period === 'day') return [start]; if (period === 'week') { start.setDate(start.getDate() - weekdayFor(start)); return Array.from({ length: 7 }, (_, index) => addDays(start, index)); } return Array.from({ length: now.getDate() }, (_, index) => new Date(now.getFullYear(), now.getMonth(), index + 1)); }
